...
class UpdateVerifikasiWisudaStatus extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $this->info("Looking for records with status 1 and tanggal_terbit older than: {$sixHoursAgo}");

            // Only process records that have tanggal_terbit (file uploaded) and are older than 6 hours
            ...
            foreach ($records as $verifikasi) {
                try {
                    ...
                    // Update status to 4 (Valid)
                    $verifikasi->update([
                        'status_id' => '4',
                        'tanggal_proses' => now()
                    ]);

                    // Create transkrip and SKPI records if they don't exist and periode_wisuda is set
