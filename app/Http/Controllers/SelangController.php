...
class SelangController extends Controller
{
    ...
    {
        $data = SelangCuti::with('status', 'tahunAkademik', 'semester')->where('user_id', Auth::user()->id)->orderBy('created_at', 'desc')->get();
    ...
    public function listStaff(Request $request)
{
    ...
    public function listAdminProdi(Request $request)
    {
        ...
        if ($request->status != 'all') {
            $list->where('status_id', $request->status);
        }
        ...
        return DataTables::of($list)
            ...
            ->editColumn('tanggal_ambil', function ($row) {
                return $row->tanggal_ambil 
                    ? Carbon::parse($row->tanggal_ambil)->translatedFormat('d F Y') . '<br/>' . 
                    Carbon::parse($row->tanggal_ambil)->translatedFormat('H:i:s') 
                    : '';
            })
            ->editColumn('tahun_akademik', function ($row) {
                return $row->tahunAkademik ?
            ...
            ->editColumn('status_id', function ($row) {
                return '<button type="button" class="btn ' . $row->status->color . ' btn-sm" disabled>' . 
                    $row->status->name . '</button>';
            })
            ->editColumn('nama_prodi', function ($row) {
                return $row->user->prodis->name ?? '-';
            })
            ...
            ->rawColumns(['action', 'tanggal_ambil', 'tahun_akademik', 'status_id', 'nama_prodi', 'catatan'])
            ->toJson();
    }
    ...
    {
        $id = decodeId($id);
        $data = SelangCuti::where('id', $id)->with('user.prodis', 'status', 'tahunAkademik', 'semester')->first();
    ...
    public function revisi(Request $request, string $id)
    {
        ...
        try {
            $id = decodeId($id);
            $ajuan = SelangCuti::findOrFail($id);
            ...
            if ($request->hasFile('file')) {
                $file = 'Selang' . '_' . Auth::user()->nim . '_' . Str::of(Auth::user()->name)->replace(' ','') . '_' . time() . '.pdf';
                $request->file('file')->storeAs('selang/upload/', $file, 'public');
    ...
    public function export(Request $request)
    {
        ...
        $name = 'Rekap_Data_Selang_Cuti_Tahun_Akademik_' . $tahun_akademik[0] . '_' . $tahun_akademik[1] . '_' . $semester->semester;
        return Excel::download(new SelangExport($tahun, $semester), $name . '.xlsx');
    }
    ...
    public function proses(Request $request, $id = null)
{
    ...
    try {
        foreach ($allIds as $id) {
            try {
                $ajuan = SelangCuti::with('user.prodis', 'status')->findOrFail($id);
                ...
                // Update nomor surat jika statusnya sesuai
                ...
                if ($request->status_id == '6' && $ajuan->file) {
                    $localPath = public_path('storage/selang/upload/' . $ajuan->file);
                    ...
                    if (file_exists($localPath)) {
                        try {
                            ...
                            $drivePath = 'Selang Cuti/' . $ajuan->file;
