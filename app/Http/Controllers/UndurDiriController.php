    ...
    public function proses(Request $request, $id = null)
{
    ...
    try {
        foreach ($allIds as $id) {
            try {
                $ajuan = UndurDiri::with('user.prodis', 'status')->findOrFail($id);
                ...
                if ($request->status_id == '6' && $ajuan->file) {
                    $localPath = public_path('storage/undur/upload/' . $ajuan->file);
                    ...
                    if (file_exists($localPath)) {
                        try {
                            ...
                            $drivePath = 'Undur Diri/' . $ajuan->file;
                ...
                // Jika status 7 (diambil), set tanggal ambil sekarang
                ...
                $status_code = 500;
                \Log::error('Error memproses data:', [
                    'error' => $th->getMessage(),
                    'id' => $id,
                    'trace' => $th->getTraceAsString()
                ]);
                continue; // Lanjut ke ID berikutnya jika error
            }
        }
