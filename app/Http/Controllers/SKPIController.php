...
class SKPIController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('pages.skpi.index', compact('tahuns', 'status', 'templates'));
    }

    public function listStaff(Request $request)
    {
        $list = SKPI::with('user.prodis', 'status')->whereRaw("SUBSTRING(periode_wisuda, 1, 4) = ?", [$request->year]);
    ...
    public function export(Request $request)
    {
        ...
        $tahun = $request->tahun;
        $name = 'Rekap_Data_SKPI_Tahun_' . $tahun;
        return Excel::download(new SKPIExport($tahun), $name . '.xlsx');
    }
    ...
    public function show($id)
    {
        try {
            $id = decodeId($id);
            $data = SKPI::where('id', $id)->with('user.prodis', 'status')->first();
            $transkrip = TranskripNilai::where('user_id', $data->user_id)->first();
            if ($transkrip->status_id == '5') {
                $data['id_transkrip'] = encodeId($transkrip->id);
                $data['no_transkrip'] = $transkrip->no_transkrip;
            }
    ...
    public function updateMany(Request $request)
    {
        try {
            ...
            SKPI::whereIn('id', $id_ajuan)->update([
                'status_id' => $status,
                'catatan' => $catatan,
            ]);
