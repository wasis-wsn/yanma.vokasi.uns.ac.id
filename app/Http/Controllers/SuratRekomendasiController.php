...
class SuratRekomendasiController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Guard when layanan not found for the current URL to avoid null dereference
        if (!$layanan) {
            Log::warning('Layanan not found for URL in SuratRekomendasiController@index', ['url' => $request->url()]);
            $templates = collect();
        } else {
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'file_ijazah' => ['nullable', 'file', 'mimes:pdf,jpg,jpeg,png,doc,docx', 'max:5120'],
            'file_transkrip' => ['nullable', 'file', 'mimes:pdf,jpg,jpeg,png,doc,docx', 'max:5120'],
        ], [
        ...
        try {
            ...
            // Handle optional file uploads for ijazah and transkrip
            if ($request->hasFile('file_ijazah')) {
                $path = $request->file('file_ijazah')->store('surat_rekomendasi/ijazah', 'public');
                $data['file_ijazah'] = $path;
            }

            if ($request->hasFile('file_transkrip')) {
                $path = $request->file('file_transkrip')->store('surat_rekomendasi/transkrip', 'public');
                $data['file_transkrip'] = $path;
            }
    ...
    public function show($id)
    {
        try {
            ...
            // Only expose URLs when files actually exist on disk
            $data->file_url = ($data->file && Storage::disk('public')->exists($data->file)) ? Storage::url($data->file) : null;
            $data->surat_hasil_url = ($data->surat_hasil && Storage::disk('public')->exists($data->surat_hasil)) ? Storage::url($data->surat_hasil) : null;
            $data->file_ijazah_url = ($data->file_ijazah && Storage::disk('public')->exists($data->file_ijazah)) ? Storage::url($data->file_ijazah) : null;
            $data->file_transkrip_url = ($data->file_transkrip && Storage::disk('public')->exists($data->file_transkrip)) ? Storage::url($data->file_transkrip) : null;

            // Debug log to help diagnose UI 'lihat ijazah/transkrip' issues
            Log::debug('SuratRekomendasi show file existence', [
                'id' => $id,
                'file' => $data->file,
                'file_exists' => $data->file ? Storage::disk('public')->exists($data->file) : false,
                'surat_hasil' => $data->surat_hasil,
                'surat_hasil_exists' => $data->surat_hasil ? Storage::disk('public')->exists($data->surat_hasil) : false,
                'file_ijazah' => $data->file_ijazah,
                'file_ijazah_exists' => $data->file_ijazah ? Storage::disk('public')->exists($data->file_ijazah) : false,
                'file_transkrip' => $data->file_transkrip,
                'file_transkrip_exists' => $data->file_transkrip ? Storage::disk('public')->exists($data->file_transkrip) : false,
            ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($isStaff) {
            // Validasi untuk Staff - hanya edit status (boleh mengupload file final)
            $request->validate([
        ...
        try {
            ...
            if ($isStaff) {
                ...
                // Log incoming request for debugging (align with SuratKeteranganAlumni)
                Log::debug('SuratRekomendasi update request (staff)', [
                    'id' => $id,
                ...
                // optional uploaded final file (store as surat_hasil)
                if ($request->hasFile('file')) {
                    // delete previous surat_hasil if exists
                    if ($surat->surat_hasil && Storage::disk('public')->exists($surat->surat_hasil)) {
                        Storage::disk('public')->delete($surat->surat_hasil);
                    }
                    $path = $request->file('file')->store('surat_rekomendasi/hasil', 'public');
                    $data_update['surat_hasil'] = $path;
                }

                // Log data prepared for update
                Log::debug('SuratRekomendasi prepared update data (staff)', [
                ...
                // handle mahasiswa uploads and replace existing files
                if ($request->hasFile('file_ijazah')) {
            ...
            $surat->refresh();
            Log::debug('SuratRekomendasi after update', [
                'id' => $surat->id,
