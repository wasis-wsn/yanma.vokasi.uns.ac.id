...
class SKLController extends Controller
{
    ...
    {
        // Decode ID dari route atau dari form
        ...
        $request->validate([
            'status_id' => ['required'],
            'no_surat' => Rule::requiredIf(fn () => in_array($request->status_id, ['3', '4', '5'])),
            'catatan' => Rule::requiredIf(fn () => in_array($request->status_id, ['2'])),
            'ids' => ['nullable', 'array'],
        ], [
        ...
        // Ambil semua ID yang akan diproses
        ...
        try {
            foreach ($allIds as $id) {
                ...
                // Set no_surat jika diperlukan
                if (in_array($request->status_id, ['3', '4', '5'])) {
                    $data['no_surat'] = $request->no_surat;
                }

                // Hapus file jika status revisi atau selesai
                if (in_array($request->status_id, ['5', '6'])) {
                    Storage::disk('public')->delete('skl/upload/' . $ajuan->lembar_revisi);
                    Storage::disk('public')->delete('skl/upload/' . $ajuan->ss_ajuan_skl);
                }

                // Upload ke GDrive jika status selesai dan file ada
                if ($request->status_id == '6' && $ajuan->lembar_revisi) {
                    $localPath = public_path('storage/skl/upload/' . $ajuan->lembar_revisi);

                    if (file_exists($localPath)) {
                        try {
                            $fileContents = file_get_contents($localPath);
                            $drivePath = 'SKL/' . $ajuan->lembar_revisi;

                            Storage::disk('google')->put($drivePath, $fileContents);

                            \Log::info('Upload ke GDrive berhasil:', [
                                'file' => $ajuan->lembar_revisi,
                                'drivePath' => $drivePath,
                            ]);
                        } catch (\Exception $e) {
                ...
                if ($request->status_id == '7') {
                    $data['tanggal_ambil'] = now();
                }

                // Update data
                $ajuan->update($data);

                \Log::info('Data SKL berhasil diproses:', [
                    'id' => $id,
                    'data_update' => $data
                ]);
            }
            ...
            return response()->json([
                ...
                'message' => count($allIds) > 1
                    ? 'Semua SKL berhasil diproses!'
                    : 'SKL berhasil diproses!',
            ]);
        } catch (\Throwable $th) {
            \Log::error('Error saat memproses data SKL:', [
                'error' => $th->getMessage(),
