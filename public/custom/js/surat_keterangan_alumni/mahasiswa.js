var table = $("#ska-datatable").DataTable({
    processing: true,
    serverSide: true,
    destroy: true,
...
setInterval(function() {
    if ($('#modalDetail').is(':visible')) {
        $.ajax({
            url: window.Laravel.queueStatus || '/suket/queue-status',
            type: "GET",
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status) {
                ...
                $("#detail-file").attr(
                    "href",
                    `${window.Laravel.baseUrl}/storage/surat_keterangan_alumni/${res.data.file}`
                );
